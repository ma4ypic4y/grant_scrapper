...
def get_info_from_block(block):
    ...
    if 'implementation_period' in d.keys():
        try:
            d['implementation_period_start'] = d['implementation_period'].split(' - ')[0]
        ...
        d['requested_amount'] = float(d['requested_amount'][:-1].replace(' ', '').replace(',','.'))
        d['co_financing'] = float(d['co_financing'][:-1].replace(' ', '').replace(',','.'))

        # для грантов победителей
        try:
            d['amount_of_costs'] = float(d['amount_of_costs'][:-1].replace(' ', '').replace(',','.'))
        except:
            d['transferred_costs'] = float(d['transferred_costs'][:-1].replace(' ', '').replace(',','.'))
...
def get_grant_data(link: str):
    ...
    grant_data['summary'] = re.sub(" +", " ",description_block.text.replace('Краткое описание','').replace('\n','').replace('\"',"'"))


    aim_block = soup.find('div', id='winner-aims')
    grant_data['aim'] = re.sub(" +", " ", aim_block.text.replace('Цель','').replace('\n','').replace('\"',"'"))
    ...
    grant_data['social_significance'] = re.sub(" +", " ", social_block.text.replace('Обоснование социальной значимости','').replace('\n','').replace('\"',"'"))

    geography_block = soup.find('div', id='winner-geography')
    grant_data['project_geography'] = re.sub(" +", " ", geography_block.text.replace('Обоснование социальной значимости','').replace('\n','').replace('\"',"'"))
    ...
    grant_data['address'] = contact_block.find('span', class_='winner__details-contacts-item').text
    grant_data['website_link'] = contact_block.find('a').text if contact_block.find('a').text != "Веб-сайт: нет" else None
...
if __name__ == '__main__':
    ...
    df['0'] = df['0'].apply(lambda x: 'https://xn--80afcdbalict6afooklqi5o.xn--p1ai'+x)
    ...
    for i in tqdm(range(0,len(all_links)-BATCH,BATCH)):
        ...
        for number in range(number_of_threads):
            t = Thread(target=insert_batch_to_elastic, args=(client,index_name,all_links[i:i+BATCH])) # get number for place in list `buttons`
            t.start()
            threads.append(t)
    ...
    if len(all_links) % BATCH:
       insert_batch_to_elastic(client,index_name,all_links[-(len(all_links) % BATCH):])
