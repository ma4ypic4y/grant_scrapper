...
def save_page_to_json(data):
    ...
    with open(
        os.path.join(DATA_PATH, str(uuid.uuid4())) + ".json", mode="w", encoding="utf-8"
    ) as f:
        json.dump(data, f, indent=4, ensure_ascii=False)
...
def get_row_from_page(driver):
    ...
    rows = driver.find_elements(
        "xpath", '//div[@class="table table--p-present table--projects"]/a'
    )
...
def get_data_from_row(driver, row):
    ...
    data["title"] = driver.find_element(
        "xpath", '//h2[@class="winner-info__title"]'
    ).text
    data["status"] = driver.find_element(
        "xpath", '//p[@class="winner-info__status"]/span'
    ).text
    money_block = driver.find_elements(
        "xpath", '//span[@class="circle-bar__info-item-number"]'
    )

    # надо переработать
    data["requested_amount"] = float(
        money_block[0].text[:-1].replace(" ", "").replace(",", ".")
    )
    data["co_financing"] = float(
        money_block[1].text[:-1].replace(" ", "").replace(",", ".")
    )
    data["amount_of_costs"] = float(
        money_block[2].text[:-1].replace(" ", "").replace(",", ".")
    )
    ...
    for el in info_block:
        ...
        try:
            ...
            pass
    try:
        data["implementation_period_start"] = data["implementation_period"].split(
            " - "
        )[0]
        data["implementation_period_end"] = data["implementation_period"].split(" - ")[
            1
        ]
    ...
    data["tasks"] = [
        task.text
        for task in driver.find_elements("xpath", '//div[@id="winner-tasks"]/ol/li')
    ]
    data["target_groups"] = [
        task.text
        for task in driver.find_elements("xpath", '//div[@id="winner-target"]/ol/li')
    ]
    data["project_geography"] = driver.find_element(
        "xpath", '//div[@id="winner-geography"]'
    ).text.split("\n")[1]
    data["social_significance"] = " ".join(
        driver.find_element("xpath", '//div[@id="winner-social"]').text.split("\n")[1:]
    )
    try:
        data["website_link"] = driver.find_element(
            "xpath",
            '//a[@class="winner__details-contacts-item winner__details-contacts-item--link"]',
        ).get_attribute("href")
    except:
        data["website_link"] = None

    data["addres"] = driver.find_element(
        "xpath", '//span[@class="winner__details-contacts-item"]'
    ).text
    data["summary"] = " ".join(
        driver.find_element("xpath", '//div[@id="winner-summary"]').text.split("\n")[1:]
    )
...
if __name__ == "__main__":
    ...
    url_parse = (
        "https://xn--80afcdbalict6afooklqi5o.xn--p1ai/public/application/cards?page=1"
    )
    ...
    for i in tqdm(range(500, 1000, number_of_threads)):
        ...
        for number in range(number_of_threads):
            t = Thread(
                target=get_data_from_page,
                args=(url_parse.replace("page=1", f"page={i+number}"),),
